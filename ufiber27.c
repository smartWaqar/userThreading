...
void UserThreading_Init(UserThreading *_uth, int _id){
    ...
    _uth->tempC = aligned_alloc(128, sizeof(Context));
...
void Scheduler(UserThreading *_uth){
  ...
  Context *tempC2 = malloc(sizeof(Context));
  ...
  while (!_uth->sp_exit_check){
      ...
      if ( t->mxcsr == flagg){
        ...
        __asm__ __volatile__(
          ...
          Context *t_p = &(_uth->buf2->context);
            //printf("Here\n");
          memcpy(t_p, _uth->tempC, sizeof(Context));
            //printf("ALso here\n");

        // __m512i zmm0;
        // __m512i zmm1;

        // zmm0 = _mm512_load_si512((const void *)_uth->tempC);
        // _mm512_store_si512((void *)t_p, zmm0);

        // zmm1 = _mm512_load_si512((const void *)(_uth->tempC + 512));
        // _mm512_store_si512((void *)(t_p + 512), zmm1);
...
//   Context *s = &(_uth->sched_thread->context);
//   Context *tempC = malloc(sizeof(Context));
